...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights, kernel_param, thre, mask_default, mask_k_default, mask_w_default, drop_prob, eta_min, node_sum, discretization=False, log=False):
        ...
        mask_w = []
        if log:
            logging.info("node_sum_init: " + str(node_sum))
        for w, k_w, op, t, m_d, m_k_d, m_w_d in zip(weights, kernel_param, self._ops, thre, mask_default, mask_k_default, mask_w_default):
            ...
            if m_d == 0:
                m = 0
            # elif w!=node_sum:
            elif torch.abs(node_sum - w)>1e-5:
                m = self.step(w - t[0])
                if m==0:
                    node_sum = node_sum-w
                    if log:
                        logging.info("w_prune: " + str(w))
                        logging.info("node_sum_prune: " + str(node_sum))
            else:
                if log:
                    logging.info("w_remain: " + str(w))
                m = m_d
            mask.append(m)
            m_k,m_w = [], []
            # if mask[k]!=0:
    ...
    def forward(self, s0, s1, weights1, weights2, kernel_param, thre, mask_default, mask_k_default, mask_w_default, drop_prob, eta_min, discretization=False, log=False):
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                try:
                    if log:
                        logging.info("weight_sum_before: " + str(weight_sum))
                        log_inner=True
                        logging.info("log3 True")
                    else:
                        log_inner=False
                    s,m,m_k,m_w = self._ops[offset + j](h, weights2[offset + j], kernel_param[offset + j+1], thre[offset + j+1], mask_default[offset + j], mask_k_default[offset + j+1], mask_w_default[offset + j], drop_prob, eta_min, weight_sum, discretization, log_inner)
                    ...
                    if log_inner:
                        logging.info("weight_sum_after: " + str(weight_sum))
                except Exception as e:
        ...
        try:
            ...
            logging.info("Error 3: " + str(e))
            idx = 0
            for s in states:
                if isinstance(s, int):
                    logging.info("int: " + str(s))
                    logging.info("idx: "+str(idx))
                idx += 1
            states = [torch.tensor([s]).cuda() if isinstance(s, int) else s for s in states]
            return torch.cat(states[2:], dim=1), mask, mask_k, mask_w
...
class Network(nn.Module):
    ...
    def forward(self, input, stage_index=0,update_mask=False, update_partial=False,gradual_discretization=False,discretization_include_current=False, log=False):
        ...
        for i, cell in enumerate(self.cells):
            ...
            try:
                if log and i==self._layers-1:
                    log_inner=True
                    logging.info("log2 True")
                else:
                    log_inner = False
                s0, [s1, m, m_k, m_w] = s1, cell(torch.Tensor(s0).cuda(), torch.Tensor(s1).cuda(), weights1, weights2, kernel_param, thre, mask, mask_k, mask_w, self.drop_path_prob, self.eta_min, discretization,log=log_inner)
            except Exception as e:
    ...
    # get the genotype with kernel num, and transform the weight mask from (cell_num,edge_num,op_num) to (cell_num,active_op_num) form
    def genotype_with_kernel_num(self):
        active_kernel_id_all = []
        kernel_num_all = []
        kernel_sum_all = []
        for cell_id in range(self._layers):
            weights2 = F.sigmoid(self._arch_parameters[cell_id])
            n = 2
            start = 0
            edge_id_global = 0
                ...
                for w_edge in W:
                    ...
                    for w_op in w_edge:
                        ...
                        if self._masks[cell_id][edge_id_global][op_id] == 1:
                            if isinstance(self.cells[cell_id]._ops[edge_id_global]._ops[op_id], SepConv) or isinstance(
                                    self.cells[cell_id]._ops[edge_id_global]._ops[op_id], DilConv):
                                num = self._masks_k[cell_id][edge_id_global + 1][op_id][-1].nelement()
                                if num >= kernel_num_max:
                                    kernel_num_max = num
                                sum_tmp += num
                                idx_num += 1
            ...
            if self.cells[cell_id].reduction_prev:
                ...
                if kernel_num_step[0] // 2 > len(idx1_1_tmp1):
                    weight, idx1_3_tmp1 = torch.sort(
                        ...
                        descending=True)
                    idx1_1_tmp1.extend(
                        [idx1_2_tmp1[i] for i in idx1_3_tmp1[:int(kernel_num_step[0] // 2 - len(idx1_1_tmp1))]])
                else:
                    weight, idx1_3_tmp1 = torch.sort(
                        F.sigmoid(self._kernel_parameters[cell_id][0][0][0][idx1_1_tmp1]),
                        descending=True)
                    ...
                    for w_op in w_edge:
                        ...
                        if self._masks[cell_id][edge_id_global][op_id] == 1:
                            ...
                            if isinstance(self.cells[cell_id]._ops[edge_id_global]._ops[op_id], SepConv) or isinstance(
                                ...
                                for l in range(len(self._masks_k[cell_id][edge_id_global + 1][op_id])):
                                    if l == 0:
                                        list = [self._masks_w[cell_id][edge_id_global][op_id][0][k].sum() for k in
                                                range(len(self._masks_w[cell_id][edge_id_global][op_id][0]))]
                                        # list = [0 for i in range(len(self._masks_w[cell_id][edge_id_global][op_id][0][0]))]
                                        # for out_idx in range(len(self._masks_w[cell_id][edge_id_global][op_id][0])):
                                        #     for in_idx in range(
                                        #             len(self._masks_w[cell_id][edge_id_global][op_id][0][out_idx])):
                                        ...
                                        idx1_1 = self.flatten(
                                            torch.nonzero(
                                                self._masks_k[cell_id][edge_id_global + 1][op_id][l]).tolist())
                                        idx1_2 = self.flatten(
                                            torch.nonzero(
                                                self._masks_k[cell_id][edge_id_global + 1][op_id][l] == 0).tolist())
                                        if kernel_num_op[l + 1] > len(idx1_1):
                                            weight, idx1_3 = torch.sort(
                                                F.sigmoid(
                                                    self._kernel_parameters[cell_id][edge_id_global + 1][op_id][l][
                                                        idx1_2]),
                                                descending=True)
                                            idx1_1.extend(
                                                [idx1_2[i] for i in idx1_3[:int(kernel_num_op[l + 1] - len(idx1_1))]])
                                        else:
                                            weight, idx1_3 = torch.sort(
                                                F.sigmoid(
                                                    self._kernel_parameters[cell_id][edge_id_global + 1][op_id][l][
                                        ...
                                        list = [self._masks_w[cell_id][edge_id_global][op_id][2][k].sum() for k in range(len(self._masks_w[cell_id][edge_id_global][op_id][2]))]

                                        weight, idx1_3 = torch.sort(torch.Tensor(list).cuda(), descending=True)
                                        idx1_1 = idx1_3[:int(kernel_num_op[3])].tolist()
                                        # idx1_1 = active_kernel_id_op[1]
                                    idx1_1.sort(reverse=False)
                                    active_kernel_id_op.append(idx1_1)
                            elif isinstance(self.cells[cell_id]._ops[edge_id_global]._ops[op_id], FactorizedReduce):
                                list = [0 for i in range(
                                    len(self.cells[cell_id]._ops[edge_id_global]._ops[op_id].conv_1.weight[0]))]
                                for out_idx in range(
                                        len(self.cells[cell_id]._ops[edge_id_global]._ops[op_id].conv_1.weight)):
                                    for in_idx in range(len(
                                            self.cells[cell_id]._ops[edge_id_global]._ops[op_id].conv_1.weight[
                                                out_idx])):
                                        list[in_idx] += abs(
                                            self.cells[cell_id]._ops[edge_id_global]._ops[op_id].conv_1.weight[out_idx][
                                                in_idx]).sum()
                                        list[in_idx] += abs(
                                            self.cells[cell_id]._ops[edge_id_global]._ops[op_id].conv_2.weight[out_idx][
                                                in_idx]).sum()
                                weight, idx1_3 = torch.sort(torch.Tensor(list).cuda(), descending=True)
                                idx1_0 = idx1_3[:int(kernel_num_step[edge_id])].tolist()
                                idx1_0.sort(reverse=False)
                                ...
                                idx1_1_tmp1 = self.flatten(
                                    torch.nonzero(self._masks_k[cell_id][edge_id_global + 1][op_id][0][
                                                  :len(self._masks_k[cell_id][edge_id_global + 1][op_id][
                                                           0]) // 2]).tolist())
                                idx1_2_tmp1 = self.flatten(
                                    torch.nonzero(self._masks_k[cell_id][edge_id_global + 1][op_id][0][
                                                  :len(self._masks_k[cell_id][edge_id_global + 1][op_id][
                                ...
                                if kernel_num_step[edge_id] // 2 > len(idx1_1_tmp1):
                                    weight, idx1_3_tmp1 = torch.sort(
                                        F.sigmoid(self._kernel_parameters[cell_id][edge_id_global + 1][op_id][0][
                                                      idx1_2_tmp1]),
                                        descending=True)
                                    idx1_1_tmp1.extend([idx1_2_tmp1[i] for i in
                                                        idx1_3_tmp1[
                                                        :int(kernel_num_step[edge_id] // 2 - len(idx1_1_tmp1))]])
                                else:
                                    weight, idx1_3_tmp1 = torch.sort(
                                        F.sigmoid(
                                            self._kernel_parameters[cell_id][edge_id_global + 1][op_id][0][
        ...
        def _parse(weights1, weights2, all_ops, cell_id, kernel_sum_all, kernel_num_all, active_kernel_id_all):
            gene = []
            masks_w_cell = []
            w_cell = []
            n = 2
            ...
            for step in range(self._steps):
                ...
                for w_edge in W:
                    ...
                    for w_op in w_edge:
                        ...
                        if self._masks[cell_id][edge_id_global][op_id]==1:
                            mask_w_op = []
                            weight_op = []
                            if isinstance(self.cells[cell_id]._ops[edge_id_global]._ops[op_id], SepConv):
                                # gene.append((all_ops[op_id], edge_id, step + 2,
                                #              [int('{:d}'.format(self._masks_k[cell_id][edge_id_global+1][op_id][0].nelement())),
                                #               int('{:d}'.format(self._masks_k[cell_id][edge_id_global+1][op_id][1].nelement())),
                                #               int('{:d}'.format(self._masks_k[cell_id][edge_id_global+1][op_id][3].nelement()))]))
                                # mask_w_tmp = self._masks_w[cell_id][edge_id_global][op_id]
                                # mask_w_op.extend(mask_w_tmp)
                                gene.append((all_ops[op_id], edge_id, step + 2,
                                          [int('{:d}'.format(len(active_kernel_id_all[edge_id_global+1][op_id][0]))),
                                           int('{:d}'.format(len(active_kernel_id_all[edge_id_global+1][op_id][1]))),
                                           int('{:d}'.format(len(active_kernel_id_all[edge_id_global+1][op_id][3])))]))
                                # gene.append((all_ops[op_id], edge_id, step + 2,
                                #              [int('{:d}'.format(
                                #                  int(kernel_num_all[edge_id_global][op_id][0]))),
                                #               int('{:d}'.format(
                                #                   int(kernel_num_all[edge_id_global][op_id][2]))),
                                #               int('{:d}'.format(
                                #                   int(kernel_num_all[edge_id_global][op_id][-1])))]))
                                for l in range(len(self._masks_k[cell_id][edge_id_global+1][op_id])):
                                    if l==0 or l==2:
                                        mask_w_op.append(self._masks_w[cell_id][edge_id_global][op_id][l][
                                            active_kernel_id_all[edge_id_global+1][op_id][l]])
                                        # logging.info(len(active_kernel_id_all[edge_id_global+1][op_id][l]))
                                        # logging.info(self._masks_w[cell_id][edge_id_global][op_id][l].shape)
                                        # logging.info(active_kernel_id_all[edge_id_global + 1][op_id][l])
                                        # logging.info(mask_w_op[l].shape)
                                    else:
                                        mask_w_op.append(self._masks_w[cell_id][edge_id_global][op_id][l][
                                            active_kernel_id_all[edge_id_global+1][op_id][l],:][:,active_kernel_id_all[edge_id_global+1][op_id][l-1]])
                                weight_op.extend([self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[1].weight[
                                            active_kernel_id_all[edge_id_global+1][op_id][0]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[2].weight[
                                            active_kernel_id_all[edge_id_global+1][op_id][1],:][:,active_kernel_id_all[edge_id_global+1][op_id][0]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[3].weight[active_kernel_id_all[edge_id_global+1][op_id][1]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[5].weight[
                                            active_kernel_id_all[edge_id_global+1][op_id][2]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[6].weight[
                                            active_kernel_id_all[edge_id_global+1][op_id][3],:][:,active_kernel_id_all[edge_id_global+1][op_id][2]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[7].weight[
                                            active_kernel_id_all[edge_id_global+1][op_id][3]]])
                                        # logging.info(self._masks_w[cell_id][edge_id_global][op_id][l].shape)
                                        # logging.info(mask_w_op[l].shape)
                            elif isinstance(self.cells[cell_id]._ops[edge_id_global]._ops[op_id], DilConv):
                                gene.append((all_ops[op_id], edge_id, step + 2,
                                             [int('{:d}'.format(
                                                 len(active_kernel_id_all[edge_id_global + 1][op_id][
                                                     0]))),
                                              int('{:d}'.format(
                                                  len(active_kernel_id_all[edge_id_global + 1][op_id][
                                                      1])))]))
                                for l in range(len(self._masks_k[cell_id][edge_id_global + 1][op_id])):
                                    if l == 0 or l == 2:
                                        mask_w_op.append(self._masks_w[cell_id][edge_id_global][op_id][l][
                                                             active_kernel_id_all[edge_id_global + 1][op_id][
                                                                 l]])
                                    else:
                                        mask_w_op.append(self._masks_w[cell_id][edge_id_global][op_id][l][
                                                         active_kernel_id_all[edge_id_global + 1][op_id][l],
                                ...
                                weight_op.extend([self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[1].weight[
                                                 active_kernel_id_all[edge_id_global + 1][op_id][0]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[2].weight[
                                             active_kernel_id_all[edge_id_global + 1][op_id][1], :][:,
                                             active_kernel_id_all[edge_id_global + 1][op_id][0]],
                                             self.cells[cell_id]._ops[edge_id_global]._ops[op_id].op[3].weight[
                                                 active_kernel_id_all[edge_id_global + 1][op_id][1]]])
                            else:
                                # if not isinstance(kernel_sum_all[edge_id], int):
                                # kernel_sum_all[edge_id] = torch.Tensor([int(kernel_sum_all[edge_id])])[0].cuda()
                                # logging.info(type(kernel_sum_all[edge_id]))
                                # logging.info(kernel_sum_all[edge_id])
                                gene.append((all_ops[op_id], edge_id, step + 2, [int('{:d}'.format(int(kernel_sum_all[edge_id])))]))
                            masks_w_cell.append(mask_w_op)
                            w_cell.append(weight_op)
                        # else:
                        #     logging.info("Have pruned: "+str(edge_id_global)+", "+str(op_id))
                        op_id += 1
                    edge_id += 1
                ...
                start = end
                n += 1
            return gene, masks_w_cell,w_cell
        ...
        for i in range(self._layers):
            ...
            #                         F.sigmoid(self._arch_parameters[i][1]).data.cpu().numpy(), all_ops, att_ops))
            # logging.info("layer: "+str(i))
            cell_init_kernel_num = self.cells[i].C
            gene, masks_w_cell, w_cell = _parse(0, F.sigmoid(self._arch_parameters[i]).data.cpu().numpy(), all_ops, cell_id=i,kernel_sum_all=kernel_sum_all[i], kernel_num_all=kernel_num_all[i], active_kernel_id_all=active_kernel_id_all[i])
            gene_list.append(gene)
            self._masks_w_final.append(masks_w_cell)
            self._w_final.append(w_cell)
        concat = range(2 + self._steps - self._multiplier, self._steps + 2)
        genotype = Genotype._make([gene_list, concat])
        return genotype